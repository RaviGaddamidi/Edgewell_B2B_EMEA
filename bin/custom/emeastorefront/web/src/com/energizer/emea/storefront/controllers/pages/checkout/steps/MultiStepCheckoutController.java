 ...
 */
package com.energizer.emea.storefront.controllers.pages.checkout.steps;
...
import com.energizer.emea.storefront.controllers.ControllerConstants;
...
public class MultiStepCheckoutController extends AbstractCheckoutStepController
{
			...
			throws CMSItemNotFoundException, CommerceCartModificationException // NOSONAR
	{
		...
		{
			String result = processCart(redirectModel);
			if (result != null) {
				return result;
			}
		}
	...
	public String getProductVariantMatrixForResponsive(@RequestParam("productCode") final String productCode, final Model model)
	{
		...
		final ProductData productData = productFacade.getProductForCodeAndOptions(productCode, Arrays.asList(ProductOption.BASIC, ProductOption.CATEGORIES));

		final CartData cartData = getCartFacade().getSessionCart();
		final String baseProductCode = productData.getBaseProduct();

		final Optional<OrderEntryData> orderEntryDataOptional =
				cartData.getEntries().stream().filter
						(orderEntryData -> orderEntryData.getProduct().getCode().equals(baseProductCode)).findFirst();

		final Map<String,ReadOnlyOrderGridData> readOnlyMultiDMap = orderGridFormFacade.getReadOnlyOrderGrid(orderEntryDataOptional.get().getEntries());
