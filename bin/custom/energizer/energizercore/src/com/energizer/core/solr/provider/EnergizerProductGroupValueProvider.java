...
/**
 * This class provides group category for the product for the solr indexing
 */
public class EnergizerProductGroupValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider
{
	private FieldNameProvider fieldNameProvider;
	private EnergizerCategoryTypeSearchService energizerCategoryTypeSearchService;
	private ConfigurationService configurationService;

	private static final Logger LOG = Logger.getLogger(EnergizerProductGroupValueProvider.class);
	...
	{
		final String ENERGIZER_GROUP = Config.getParameter("marketingcategory.level2.type");
				...
				for (final CategoryModel category : superCategories)
				{
					if (category instanceof EnergizerCategoryModel)
					{
						final List<EnergizerCategoryModel> allCategoriesAsSegments = energizerCategoryTypeSearchService
								.getEnergizerCategoryWithType((EnergizerCategoryModel) category, ENERGIZER_GROUP);
						...
						if (castedCategoryModel.getCategoryType() != null
								&& castedCategoryModel.getCategoryType().equals(ENERGIZER_GROUP))
						{
							...
							if (indexedProperty.isLocalized())
							{
								...
								for (final LanguageModel language : languages)
								{
									...
									for (final String fieldName : names)
									{
										...
										LOG.info("Group added  :" + segment.getName(new Locale(language.getIsocode())));
									}
