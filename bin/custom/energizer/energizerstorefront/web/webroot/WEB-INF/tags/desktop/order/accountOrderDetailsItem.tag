...
<c:choose>
	<c:when test="${not inProgress}">
		<c:if test="${consignment.status.code eq 'READY_FOR_PICKUP'}" >
			<h2><spring:theme code="text.accout.order.warning.storePickUpItems" text="Reminder - Please pick up your items(s) soon."/></h2>
		</c:if>
	</c:when>
</c:choose>
...
<c:choose>
	<c:when test="${consignment.status.code eq 'READY_FOR_PICKUP'}" >
		...
		<c:url value="/store/${pos.name}" var="posUrl"/>
			<strong> Pick Up Location:</strong> <a href="${posUrl}" >${pos.name}</a>
	</c:when>
</c:choose>
