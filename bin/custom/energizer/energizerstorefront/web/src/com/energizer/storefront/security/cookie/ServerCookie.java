 ...
 *  
 */
package com.energizer.storefront.security.cookie;
...
/**
 * Stripped out org.apache.tomcat.util.http.ServerCookie for use of #appendCookieValue(StringBuffer, int, String, String, String,
 * String, String, int, boolean, boolean)
 */
@SuppressWarnings("PMD")
// As this code is coped from org.apache.tomcat.util.http.ServerCookie and not written to hybris standards it does not meet PMD
// rules
public class ServerCookie implements Serializable
{
			...
			final boolean isHttpOnly)
	{
		...
		/*
		 * The spec allows some latitude on when to send the version attribute with a Set-Cookie header. To be nice to clients,
		 * we'll make sure the version attribute is first. That means checking the various things that can cause us to switch to a
		 * v1 cookie first.
		 * 
	...
	public static final class CookieSupport
	{
		...
		/**
		 * If true, separators that are not explicitly dis-allowed by the v0 cookie spec but are disallowed by the HTTP spec will
		 * be allowed in v0 cookie names and values. These characters are: \"()/:<=>?@[\\]{} Note that the inclusion of / depends
		 * on the value of {@link #FWD_SLASH_IS_SEPARATOR}.
		 */
		...
		/**
		 * The list of separators that apply to version 0 cookies. To quote the spec, these are comma, semi-colon and white-space.
		 * The HTTP spec definition of linear white space is [CRLF] 1*( SP | HT )
		 */
		private static final char[] V0_SEPARATORS = { ',', ';', ' ', '\t' };
		private static final boolean[] V0_SEPARATOR_FLAGS = new boolean[128];

		/**
		 * The list of separators that apply to version 1 cookies. This may or may not include '/' depending on the setting of
		...
		static
		{
			...
			/*
			 * Excluding the '/' char by default violates the RFC, but it looks like a lot of people put '/' in unquoted values:
			 * '/': ; //47 '\t':9 ' ':32 '\"':34 '(':40 ')':41 ',':44 ':':58 ';':59 '<':60 '=':61 '>':62 '?':63 '@':64 '[':91
			 * '\\':92 ']':93 '{':123 '}':125
			 */
