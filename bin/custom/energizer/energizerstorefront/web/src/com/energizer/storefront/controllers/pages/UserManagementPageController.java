...
public class UserManagementPageController extends MyCompanyPageController
{
			...
			throws CMSItemNotFoundException
	{
		...
		energizerCompanyB2BCommerceFacade.populateUserRoles(searchPageData);
	...
	public String createUser(final Model model) throws CMSItemNotFoundException
	{
		 ...
		 *      that logged in user.
		 **/
		final EnergizerB2BUnitModel b2bModel = energizerCompanyB2BCommerceFacade.getEnergizerB2BUnitModelForLoggedInUser();
		final boolean validateUserCountFlag = energizerCompanyB2BCommerceFacade.validateUserCount(b2bModel);
		if (validateUserCountFlag)
		{
			storeCmsPageInModel(model, getContentPageForLabelOrId(MY_COMPANY_CMS_PAGE));
			setUpMetaDataForContentPage(model, getContentPageForLabelOrId(MY_COMPANY_CMS_PAGE));
			final List<Breadcrumb> breadcrumbs = myCompanyBreadcrumbBuilder.createManageUserBreadcrumb();
